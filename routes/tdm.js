 ...
 *  Returns mid-current grid with references to all subnets
 */
router.use('/test/overview', function (req, res, next) {
    ...
    const fs = require('fs');
    //joining path of directory
    const directoryPath = path.join(__dirname, '../public/test_topology');
    //passsing directoryPath and callback function
    fs.readdir(directoryPath, function (err, files) {
        //handling error
        if (err) {
...
router.use('/test/:id', function (req, res, next) {
    ...
    var fs = require('fs');
    fs.readFile( __dirname + '/../public/test_topology/' + req.params.id + '.json','utf8', function (err, data) {
        if (err) {
            console.log(err);
            res.setHeader('Content-Type', 'application/json');
